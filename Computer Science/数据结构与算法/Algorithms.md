...
* At lest *M* / 2 key-link pairs in other nodes.
...
* **Proposition**: A search or an insertion in a B-tree of order *M* with *N* keys requires between log*<sub>M</sub>*​<sub>-1</sub> *N* and log *<sub>M</sub>*​<sub>/2</sub> *N* probes.
* Pf. All internal nodes (besides root) have between *M* / 2 and *M* - 1links.
...
The code below tries to relax edge from every vertex in one time, then connect in second or more round.
...
* Proposition: If any vertex *v* is updated in phase *v*, there exists a neagtive cycle (and can trace back `edgeTo[v]`​ entries to find it).
...
Challenge: Use less memory. (e.g., 65536 way tries for Unicode).
